...
import { PhonepeEvent } from './phonepe-event.schema';
...
@Injectable()
export class PhonepeEventService {
  constructor(
    @InjectModel('PhonepeEvent')
    protected readonly model: Model<PhonepeEvent & Document>
  ) {}

  async findAll(model: MongooseQueryModel): Promise<PhonepeEvent[]> {
  ...
  async findById(id: string): Promise<PhonepeEvent> {
    const query = this.model.findById(new Types.ObjectId(id));

    return query.exec();
  }

  async findOne(model: MongooseQueryModel): Promise<PhonepeEvent> {
  ...
  async insert(doc: Partial<PhonepeEvent>, session: ClientSession): Promise<any> {
