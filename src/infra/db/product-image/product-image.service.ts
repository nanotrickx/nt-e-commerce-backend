...
export class ProductImageService {
  constructor(
    ...
    protected readonly model: Model<ProductImage & Document>
  ) {}

  async findAll(model: MongooseQueryModel): Promise<ProductImage[]> {
  ...
  async findOne(model: MongooseQueryModel): Promise<ProductImage> {
  ...
  async insert(doc: Partial<ProductImage>, session: ClientSession): Promise<any> {
  ...
  async pagedAsync(
    ...
    filter: { searchKey?: any, productId?: string } = {}
    ...
    if (filter.productId) {
      queryFilter.product_id = filter.productId;
    }
