...
export class ProductService {
  constructor(
    @InjectModel('Product')
    protected readonly model: Model<Product & Document>
  ) {}

  async findAll(model: MongooseQueryModel): Promise<Product[]> {
  ...
  async findOne(model: MongooseQueryModel): Promise<Product> {
  ...
  async insert(doc: Partial<Product>, session: ClientSession): Promise<any> {
  ...
  async pagedAsync(
    ...
    filter: { searchKey?: any, product_type?: string, status?: string } = {}
    ...
    if(filter.product_type){
      queryFilter.product_type = filter.product_type;
    }
    if(filter.status){
