...
import { StripeEvents } from './stripe-events.schema';
...
export class StripeEventsService {
  constructor(
    @InjectModel('StripeEvents')
    protected readonly model: Model<StripeEvents & Document>
  ) {}

  async findAll(model: MongooseQueryModel): Promise<StripeEvents[]> {
  ...
  async findById(id: string): Promise<StripeEvents> {
    const query = this.model.findById(new Types.ObjectId(id));

    return query.exec();
  }

  async findOne(model: MongooseQueryModel): Promise<StripeEvents> {
  ...
  async insert(doc: Partial<StripeEvents>, session: ClientSession): Promise<any> {
  ...
  async insertMany(
    doc: Partial<StripeEvents[]>,
    session: ClientSession
  ): Promise<StripeEvents[]> {
    return await this.model.create(doc, {
      session
    });
  }
